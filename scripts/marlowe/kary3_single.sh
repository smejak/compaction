...
#SBATCH --error=logs/kary3_%x_%j.err
#SBATCH --output=logs/kary3_%x_%j.out

# k=3 stacking probe (rope_shift) over patients 01,03,04 — the cumulative
# k=3 prefix of the canonical k=5 patient subset. Companion to kary_single.sh
# (k=5) and kary4_single.sh (k=4); together they fill in the naive-vs-rope
...
"$PY" -u scripts/run_kary_experiment.py \
    --patients patient_01,patient_03,patient_04 \
    --variant rope_shift \
