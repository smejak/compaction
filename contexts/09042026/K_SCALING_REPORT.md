...
generated by `scripts/plot_kary_scaling.py` (committed in `d2c8f98`). The
script is idempotent and auto-discovers any new k results, so it can be
re-run later when additional points land.
